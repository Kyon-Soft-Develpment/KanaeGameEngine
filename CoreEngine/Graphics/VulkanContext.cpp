...
void VulkanContext::initialize(const std::string& windowTitle) {
    // Initialize GLFW
    if (!glfwInit()) {
    ...
    window = glfwCreateWindow(1280, 720, windowTitle.c_str(), nullptr, nullptr);
    if (!window) {
        glfwTerminate();
    ...
    appInfo.pEngineName = "Kanae Engine";
    ...
    // Only enable validation layer if present
    ...
    // Only enable portability extension if present
    ...
    if (portabilityFound) {
        ...
        std::cerr << "[Vulkan] Portability extension not found, continuing without it." << std::endl;
    }

    // Only enable debug utils if present
    ...
    physicalDevice = devices[0]; // Just take the first device for now
    ...
    graphicsQueueFamily = 0; // Just use the first queue family for now
    ...
    presentQueue = graphicsQueue; // For simplicity, use the same queue
